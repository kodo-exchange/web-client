...
import { ETHERSCAN_URL, TOP_ASSETS } from '../../stores/constants'
...
export default function AssetSelect({ type, value, assetOptions, onSelect }) {
  ...
  const renderAssetOption = (type, asset, idx) => {
    return (
      <li
        className="flex w-full flex-row items-center gap-2 hover:bg-bg-light pl-3 pr-3 py-2 text-sm"
        key={asset.address + '_' + idx}
        onClick={() => {
          onLocalSelect(type, asset)
        }}
      >
        <div className="">
          <PlusCircleIcon
            onClick={(event) => {
              ...
              addTokenToMetaMask(asset)
            }}
            className="h-5 w-5 text-text-gray cursor-pointer"
          />
        </div>
        <div className="relative flex h-[36px] w-[36px] shrink-0 flex-row justify-between items-center">
          <img
            className="aspect-square absolute left-0 z-0 rounded-full"
            src={asset ? `${asset.logoURI}` : ''}
            width="36"
            height="36"
            alt=""
        ...
        <div className="grow flex flex-col items-start normal-case">
            ...
            {asset?.address != 'ETH' && (
              <>
                <span className="flex items-center">
                  <a
                    data-tooltip-id={`bribe-asset-select-tooltip-${asset.address + '_' + idx}`}
                ...
                <Tooltip
                  id={`bribe-asset-select-tooltip-${asset.address + '_' + idx}`}
          ...
          <div className="text-[10px] sm:text-xs text-text-gray">{asset ? asset.name : ''}</div>
        </div>
        <div className="flex flex-col items-end normal-case  font-normal">
          <div className="text-sm text-white">{asset && asset.balance ? formatCurrency(asset.balance) : '0.00'}</div>
          <div className="text-[10px] sm:text-xs text-text-gray">Balance</div>
        </div>
      </li>
    )
  }
          ...
          >
            <span className="relative h-4 w-4 items-center">
              ...
              {/* <Image
                ...
                width={16} // replace with your desired width
                height={16} // replace with your desired height
              /> */}
            </span>
